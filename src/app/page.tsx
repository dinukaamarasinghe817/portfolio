...
// const swiper: SwiperType = new SwiperType({...});
...
import openhack from '../assets/awards/openhack.png';
...
export default function Home() {
  ...
  // Use a Record to type the refs for each section
  ...
  //     console.log("hellow");
  //     const offsets = Object.entries(sectionsRef).map(([key, ref]) => ({
  //       section: key as Section,
  //       offsetTop: ref.current?.offsetTop ?? 0,
  //     }));

  //     const scrollPosition = window.scrollY + window.innerHeight / 30;

  //     const currentSection = offsets
  //       .reverse()
  //       .find(({ offsetTop }) => scrollPosition >= offsetTop)?.section;
  ...
  return (
    ...
    <div className="flex flex-col gap-12 pt-32 max-w-[640px] mx-4">
      <nav className="fixed top-1/2 max-w-[52px] bg-grey left-[150px] transform -translate-y-1/2 flex flex-col gap-3 items-start overflow-x-show rounded-[50px] p-[6px] z-10">
        <a 
        onClick={() => handleNavigation("home" as Section)}
        className="flex flex-row gap-4 items-center justify-start rounded-full transition cursor-pointer">
          ...
          <span className={`${activeLink === 'home' ? 'flex' : 'hidden'}`}>Home</span>
        </a>
        <a 
        onClick={() => handleNavigation("projects" as Section)}
        className="flex flex-row gap-4 items-center justify-start mb-0 rounded-full transition cursor-pointer">
          ...
          <span className={`${activeLink === 'projects' ? 'flex' : 'hidden'}`}>Projects</span>
        </a>
        <a 
        ...
        className="flex flex-row gap-4 items-center justify-start mb-0 rounded-full transition cursor-pointer">
          ...
          <span className={`${activeLink === 'stack' ? 'flex' : 'hidden'}`}>Stack</span>
        </a>
        <a 
        ...
        className="flex flex-row gap-4 items-center justify-start mb-0 rounded-full transition cursor-pointer">
          ...
          <span className={`${activeLink === 'blogs' ? 'flex' : 'hidden'}`}>Blogs</span>
        </a>
        <a 
        ...
        className="flex flex-row gap-4 items-center justify-start mb-0 rounded-full transition cursor-pointer">
          ...
          <span className={`${activeLink === 'awards' ? 'flex' : 'hidden'}`}>Awards</span>
        </a>
      </nav>
      ...
      <section id="awards" className="overflow-visible" ref={sectionsRef.awards}>
        ...
        >
          {awards.map((award, index) => (
            ...
            >
              <img className="rounded-[8px] w-[400px] h-[250px]" src={award.imagePath} alt="slide_image" />
